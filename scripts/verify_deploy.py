...
def fetch(url, timeout):
    """Return (status, body_bytes). Raises only for transport-level failures."""
...
def fetch_root(url, warmup_seconds, timeout):
    ...
    A 4xx is definitive: the app answered and said this URL is not there, and no amount
    of waiting changes that. Retrying it would burn the whole warm-up budget to reach
    the same verdict.
    """
...
def main(argv=None):
    ...
    body = fetch_root(base, args.warmup_seconds, args.timeout)
    ...
    for url in resolved:
        try:
            status, _ = fetch(url, args.timeout)
