...
- [ ] 3.1 App registration `gh-tenexcards-deploy` exists and returns one `appId`
- [ ] 3.2 `Contributor` role assignment present at resource-group scope
- [ ] 3.3 Federated credential lists the exact `refs/heads/main` subject
- [ ] 3.4 `gh secret list` shows all three secret names
- [ ] 3.5 No role assignment exists at subscription scope
- [ ] 3.6 `az ad app credential list` returns nothing — no client secret was created

#### Manual

- [ ] 3.7 Subject string matches `gh repo view` character for character
- [ ] 3.8 If the contingency fired, the reason is written down for Phase 5
- [ ] 3.9 If the session gate stopped the phase, the user ran `az login` — the agent did not
